...
BEGIN
    --INSERCION DE DATOS EN LA TABLA DE REPORTE
...
EXCEPTION
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('ERROR AL INSERTAR EL REPORTE: ' || SQLERRM);
END;
/
...
CREATE OR REPLACE PROCEDURE PRC_PROCESO_MASIVO_MULTAS (
    ...
    --  VARIABLES PARA LOGICA DE CALUCLOS
    v_conteo_pagos          NUMBER;
    v_conteo_reincidencia   NUMBER;
    v_multa_uf              NUMBER;
...
BEGIN
    ...
    -- SE TRANSFORMA INGRESADA A FORMATO NUMER(6)
    ...
    LOOP
        ...
        IF v_conteo_pagos = 0 THEN
            ...
            SELECT COUNT(*) INTO v_conteo_reincidencia
            FROM pago_gasto_comun
            ...
            IF v_conteo_reincidencia = 0 THEN
            ...
            ELSE
                ...
                v_observacion_texto := 'Se realizara el corte del Combustible y agua a contar del ' || TO_CHAR(v_reg.fecha_pago_gc, 'DD/MM/YYYY');
            END IF;
...
DECLARE
--DEFINIMOS LOS PARAMETROS DE PRUEBA
    V_UF_PRUEBA     NUMBER := 29509;
    V_FECHA_PRUEBA DATE := TO_dATE('31/05' || TO_CHAR(SYSDATE, 'YYYY'), 'DD/MM/YYYY');
BEGIN
    DBMS_OUTPUT.PUT_LINE('--- EJECUTANDO PRUEBA ---');
    --LLAMAMOS AL PROCEDIMIENTO PRINCIPAL
    prc_proceso_masivo_multas(V_UF_PRUEBA,V_FECHA_PRUEBA);
    DBMS_OUTPUT.PUT_LINE('--- EJECUTANDO PRUEBA ---');
end;
/
...
SELECT 
    ...
    ID_EDIF, 
    NRO_DEPTO, 
    FECHA_DESDE_GC,  -- Probablemente muestran fechas
    FECHA_HASTA_GC, 
    MULTA_GC         -- Y el valor de la multa
FROM GASTO_COMUN
WHERE ANNO_MES_PCGC = 202605  -- Filtra solo el mes actual (Mayo)
  AND MULTA_GC > 0            -- Filtra solo los que tienen multa (Pago Cero)
